...
### Sample the Data and Create CSV Files
...
# Create dataframes and write CSV files with user nodes, song nodes, and ratings at each sample level
for (i in samp_lvls[i:length(samp_lvls)]) {
  
  # Take a random sample of user IDs
  user_nodes <- sample(user_list$user_id, i, replace = F)
  names(user_nodes) <- c("user_id")

  # Subset the full dataframe by the random user IDs. Remove duplicate ratings.
  ratings <- distinct(subset(joined_final, joined_final$user_id %in% user_nodes))
  ratings <- select(ratings, user_id, rating, title)
  
  # Get the unique list of songs contained in the sample.
  song_nodes <- distinct(as.data.frame(ratings$title))
  names(song_nodes) <- c("title")
  
  # Make unique file names for each sample level
  user_filename <- sprintf('users_%s.csv', i)
  rating_filename <- sprintf('ratings_%s.csv', i)
  song_filename <- sprintf('songs_%s.csv', i)
  
  # Write the files as a CSV
  write.csv(user_nodes, file = user_filename, row.names = F)
  write.csv(ratings, file = rating_filename, row.names = F)
  write.csv(song_nodes, file = song_filename, row.names = F)
  
  # Success message
  message <- sprintf('Created CSV Files For %d-User Sample', i)
  print(message)
