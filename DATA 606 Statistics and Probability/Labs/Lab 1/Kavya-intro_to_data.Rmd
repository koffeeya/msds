...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    There are 20,000 cases in this data set and 9 variables in this dataset.
...
2.  Create a numerical summary for `height` and `age`, and compute the 
    ...
    There are 9,569 males in this sample. 23% of the sample reports being in excellent health.
...
3.  What does the mosaic plot reveal about smoking habits and gender?
    ...
    The mosaic plot reveals that more males have smoked at least 100 cigarettes than females.
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    The box plot shows that as respondents rated themselves as having poorer health, their BMI increases.
    
    I chose to plot gender and BMI. It seems like males have a higher average BMI than females, although females have more outliers.
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    There seems to be a slight upward trend to the data. As a person's current weight increases, their desired weight also increases, but it is not 1:1. For example, the majority of people with a current weight of 300 lbs do not also desire to be 300 lbs.
    
```{r}
qplot(cdc$weight, cdc$wtdesire)
```
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    wdiff is continuous numerical data. If an observation is 0, it means that the person's current weight and desired weight are the same. If an observation is positive, it means that the person wants to weigh more than their current weight. If an observation is negative, it means that the person wants to weigh less than their current weight. 
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    The median of wdiff is -10 lbs, and the mean is -14.59 lbs.
    
    The shape of the data appears to be normal around the 0 lb mark.
    
    In general, most people are not satisfied with their current weight and want to lose about 10 lbs.
...
wdiff_males <- males$wtdesire - males$weight
wdiff_females <- females$wtdesire - females$weight

summary(wdiff_females)
summary(wdiff_males)

boxplot(wdiff_females, wdiff_males)
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    71% of observations are within one standard deviation of the mean weight of 169.7 lbs.
...
a <- subset(cdc, abs(cdc$weight - mean(cdc$weight)) < sd(cdc$weight))
